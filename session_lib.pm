...
sub get_db_handle
{
	...
	(my $connect_string,
	 ...
	 my $db_password) = db_info($db_alias);
        ...
        # Force DBI Pg driver into utf8 mode. 
        # Something about  pg_utf8_strings => 1, but man DBB::Pg doesn't mention it.
	my $dbargs = {AutoCommit => 0, PrintError => 1, pg_enable_utf8 => 1};
...
sub sqlite_db_handle
{
    ...
    if (! exists($db_handles{$db_alias}))
    {
        ...
        # jul 1 2015 Enable unicode aka utf8 mode

        my $dbargs = {sqlite_use_immediate_transaction => 1, AutoCommit => 0, PrintError => 1, sqlite_unicode => 1};
